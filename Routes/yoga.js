...
router.post("/login",async(req,res)=>{
    ...
    if(!Email || !Password || !Date){
       ...
       if(user){
        ...
        if(!match){
            return res.json("Enter credentials are incorrect")
        }
        else{
        ...
        return res.json("No user is registred with this emailId")
       } 
    }
})
router.patch("/update",async (req,res)=>{
    ...
    if(Number(Newdate[2]) != Number(olddate[2]) || Number(Newdate[1])!=Number(olddate[1])){
        ...
        return res.json("Batch transferred succesfully")
    }
    else{
        return res.json("Batch can be transferred in next month")
    }
})
